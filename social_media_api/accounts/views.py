...
from .serializers import (
    UserSerializer, 
    RegistrationSerializer,  # Corrected import name: RegistrationSerializer
    LoginSerializer
)
...
class RegistrationView(generics.CreateAPIView):
    ...
    """
    queryset = User.objects.all()
    # Using the corrected serializer name
    ...
    def perform_create(self, serializer):
        # Creates the user
        user = serializer.save()
        # Create a token for the new user immediately upon successful registration
...
class LoginView(APIView):
    ...
    Accepts POST requests with username and password, returns an auth token.
    """
    permission_classes = (permissions.AllowAny,)
...
class UserProfileView(generics.RetrieveUpdateDestroyAPIView):
    """
    Endpoint for viewing, updating, and deleting the authenticated user's profile.
    """
...
class UserPublicProfileView(generics.RetrieveAPIView):
    """
    Endpoint for viewing any user's public profile by their username.
    """
    queryset = User.objects.all()
    serializer_class = UserSerializer
    lookup_field = 'username' # Allows lookup by username in the URL
    permission_classes = (permissions.AllowAny,)
